...
const KIND_CHIP: Record<EventKind, string> = {
  activity: "bg-sky-100 text-sky-900",
  maintenance: "bg-amber-100 text-amber-900",
  dining: "bg-emerald-100 text-emerald-900",
  housekeeping: "bg-rose-100 text-rose-900",
};

export default function PhoneCalendarPage() {
            ...
            >
              ...
              <ul className="mt-1 space-y-0.5">
                {events.slice(0, 2).map((item) => (
                  <li
                    key={item.id}
                    className={`truncate rounded px-1 py-0.5 text-[10px] font-semibold ${KIND_CHIP[item.kind]}`}
                  >
                    {item.title}
                  </li>
                ))}
              ...
              <li key={item.id} className="rounded-xl bg-slate-50 px-3 py-3">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    ...
                    <p className="text-xs text-slate-500">
                      {EVENT_KIND_LABEL[item.kind]} · {item.allDay ? "All day" : formatTime(item.startsAt)}
