  ...
  return (
    <div className="mb-5">
      <h1 className="flex items-center gap-2 text-2xl font-semibold text-slate-900">
        <i className={`fas ${icon} text-base text-teal-600`} aria-hidden />
        {title}
      </h1>
...
export function LevelTag({ level }: { level: "urgent" | "watch" | "bottleneck" | "on_track" | "no_data" }) {
  const styles = {
    urgent: "bg-rose-700 text-white",
    bottleneck: "bg-rose-700 text-white",
    watch: "bg-amber-100 text-amber-900",
    on_track: "bg-teal-100 text-teal-800",
  ...
  return (
    <span className={`inline-flex rounded px-1.5 py-0.5 text-[9px] font-bold uppercase tracking-wide ${styles[level]}`}>
      {labels[level]}
    </span>
  );
}
...
export function Segmented<T extends string>({
  ...
  return (
    <div role="group" aria-label={label} className="inline-flex rounded-full bg-slate-200/70 p-1">
      {options.map((o) => (
        <button
          key={o.value}
          type="button"
          ...
          className={`rounded-full px-4 py-1.5 text-xs font-bold transition ${
            value === o.value ? "bg-teal-900 text-white" : "text-slate-700"
          }`}
        >
          {o.label}
        </button>
